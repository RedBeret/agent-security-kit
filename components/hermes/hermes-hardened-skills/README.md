...
```bash
git clone https://github.com/RedBeret/hermes-hardened-skills.git \
  ~/.hermes/projects/hermes-hardened-skills
cd ~/.hermes/projects/hermes-hardened-skills
bash setup.sh
```

Or point Hermes at the repo directly (auto-updates with `git pull`):

```yaml
# ~/.hermes/config.yaml
skills:
  external_dirs:
    - ~/.hermes/projects/hermes-hardened-skills/skills
```
...
```bash
vim ~/.hermes/skills/cybersecurity-advisor/SKILL.md
```
