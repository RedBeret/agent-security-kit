#!/usr/bin/env bash
# hermes-memory-encrypt: Encrypt MEMORY.md and USER.md at rest using AES-256-CBC.
# Key from OS keystore (HERMES_MEMORY_KEY). Includes HMAC integrity verification.
#
# Features:
...
MEMORIES_DIR="${HERMES_HOME:-$HOME/.hermes}/memories"
BACKUP_DIR="${HERMES_HOME:-$HOME/.hermes}/backups/memory"
_os="$(uname -s)"

# ── Get encryption key ─────────────────────────────────────────────────
_get_key() {
  if [ -n "${HERMES_MEMORY_KEY:-}" ]; then
    echo "$HERMES_MEMORY_KEY"
    return 0
  fi
  case "$_os" in
    Darwin)
      security find-generic-password -a "$USER" -s "HERMES_MEMORY_KEY" -w 2>/dev/null
      ;;
    Linux)
      if command -v secret-tool >/dev/null 2>&1; then
        secret-tool lookup service hermes key HERMES_MEMORY_KEY 2>/dev/null
      elif command -v pass >/dev/null 2>&1; then
        pass show hermes/HERMES_MEMORY_KEY 2>/dev/null | head -1
      fi
      ;;
  esac
}
...
KEY="$(_get_key)"
if [ -z "$KEY" ]; then
  echo "✗ No HERMES_MEMORY_KEY found in keystore. Run setup.sh first."
  exit 1
fi

# Validate key is 64 hex chars (256-bit)
if ! echo "$KEY" | grep -qE '^[0-9a-fA-F]{64}$'; then
  echo "✗ HERMES_MEMORY_KEY must be a 64-character hex string (256-bit)"
  exit 1
fi
