#!/usr/bin/env bash
# hermes-memory-encrypt: Decrypt MEMORY.md and USER.md for Hermes to read.
# Verifies HMAC-SHA256 integrity before decrypting (detects tampering).
# Call before starting Hermes. Skips if plaintext already exists.
...
KEY="$(_get_key)"
if [ -z "$KEY" ]; then
  echo "✗ No HERMES_MEMORY_KEY found — memories stay encrypted"
  exit 1
fi
