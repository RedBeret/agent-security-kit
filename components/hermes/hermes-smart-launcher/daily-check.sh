#!/usr/bin/env bash
# hermes-smart-launcher: Daily health check
...
if [ -n "${HERMES_PORT:-}" ]; then
  if curl -s --max-time 2 "http://127.0.0.1:$HERMES_PORT/" >/dev/null 2>&1; then
    echo -e "${GREEN}✓${NC} Agent gateway running on :$HERMES_PORT"
  else
