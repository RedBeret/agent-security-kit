...
AGENT_CMD="${HERMES_CMD:-hermes gateway}"
AGENT_PORT="${HERMES_PORT:-}"
AGENT_VENV="${HERMES_VENV:-$AGENT_HOME/hermes-agent/venv}"
...
if [ -f "$AGENT_HOME/load-secrets.sh" ]; then
  HERMES_QUIET_SECRETS=1 source "$AGENT_HOME/load-secrets.sh" 2>/dev/null || true
  log "  ${GREEN}ok${NC} secrets loaded from keystore"
elif [ -f "$AGENT_HOME/.env" ]; then
  set -a; source "$AGENT_HOME/.env"; set +a
  log "  ${YELLOW}--${NC} loaded .env (consider migrating to hermes-secret-store)"
else
...
if [ "$HEAVY" -eq 1 ]; then
  ...
  if command -v hermes &>/dev/null; then
    CUR_VER=$(hermes --version 2>/dev/null | head -1 || echo "unknown")
    log "  hermes: $CUR_VER"
    pip install --upgrade hermes-agent --break-system-packages -q 2>/dev/null && \
      log "  ${GREEN}ok${NC} hermes updated" || \
      log "  ${YELLOW}--${NC} hermes update failed"
  fi
