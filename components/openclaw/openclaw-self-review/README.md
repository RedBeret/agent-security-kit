...
```bash
git clone https://github.com/RedBeret/openclaw-self-review.git \
  ~/.openclaw/projects/openclaw-self-review
cd ~/.openclaw/projects/openclaw-self-review
bash setup.sh
```

Optional hook in `~/.openclaw/openclaw.json`:

```jsonc
"hooks": {
  "pre_llm_call": [
    - command: "~/.openclaw/agent-hooks/inject-review-reminder.sh"
"timeout": 3
```
