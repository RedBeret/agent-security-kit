#!/usr/bin/env bash
# openclaw-secret-store: Store a key in OS keystore
...
case "$_os" in
  ...
  Linux)
    if command -v secret-tool >/dev/null 2>&1; then
      echo -n "$KEY_VALUE" | secret-tool store --label="OpenClaw: $KEY_NAME" service openclaw key "$KEY_NAME"
    elif command -v pass >/dev/null 2>&1; then
      echo "$KEY_VALUE" | pass insert -e "openclaw/$KEY_NAME"
    else
